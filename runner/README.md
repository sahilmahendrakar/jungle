...
|---|---|
| `jungle-runner connect` | authenticate + register this device, then run the daemon |
| `jungle-runner up` | run the daemon on an already-connected device (e.g. at login) |
| `jungle-runner status` | show this device's registration |
| `jungle-runner logout` | disconnect this device locally (also remove it from the web app to fully revoke) |

Config and per-agent workspaces live under `~/.jungle-runner/`. Requires Node 18+
...
[`../docs/runner-protocol.md`](../docs/runner-protocol.md). The `jungle-runner`
