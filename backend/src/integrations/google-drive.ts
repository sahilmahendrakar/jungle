...
export const googleDriveAdapter: IntegrationAdapter = {
  ...
  // Bind to one of the attaching user's Drive connections — the picker's choice
  ...
  async resolveConfig(ctx, rawConfig): Promise<Record<string, unknown>> {
    ...
    if (connectionId) {
      ...
      if (!conn || conn.participant_id !== ctx.me.id || conn.integration_key !== KEY) {
        throw new ApiError(400, "that Google Drive connection doesn't belong to you");
      }
    ...
    if (conns.length === 0) throw new ApiError(400, "connect your Google Drive account in Settings first");
    if (conns.length > 1) throw new ApiError(400, "choose which Google Drive account this agent should use");
