...
| `memory` | `content` | the agent's `/workspace/MEMORY.md` mirror — full current text (`""` = absent/empty). Sent after any turn that changed it (hash-compared) and once after each `configure`. Backend persists it on the participant row (served by `GET /api/agents/:id/memory`) and broadcasts `agent_memory_changed` |
